...
def test_openai_gpt_5_6_model_info(model):
    ...
    assert bool(info.get("supports_ultra_reasoning_effort")) is (model != "gpt-5.6-luna")
...
def test_get_model_info_exposes_gpt_5_6_capabilities(monkeypatch):
    json_path = Path(__file__).parents[2] / "model_prices_and_context_window.json"
    with open(json_path) as f:
        model_cost = json.load(f)
    ...
    info = litellm.get_model_info("gpt-5.6-sol", custom_llm_provider="openai")
