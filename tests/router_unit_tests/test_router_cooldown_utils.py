...
@pytest.mark.parametrize("exception_status", [429, "429"])
def test_is_cooldown_required_rate_limit_status(
    testing_litellm_router, exception_status
):
    result = _is_cooldown_required(
        litellm_router_instance=testing_litellm_router,
        model_id="test_deployment",
...
@pytest.mark.parametrize(
    "exception_status",
    [401, 402, 404, 408, 500, 502, 503, 504],
)
def test_is_cooldown_required_rejects_non_rate_limit_statuses(
    testing_litellm_router, exception_status
):
    result = _is_cooldown_required(
        litellm_router_instance=testing_litellm_router,
...
def test_set_cooldown_deployments_ignores_server_disconnected():
    router = Router(
        model_list=[
            {
                "model_name": "gpt-3.5-turbo",
    ...
    cooled_down = _set_cooldown_deployments(
        ...
        original_exception=litellm.exceptions.InternalServerError(
            "Server disconnected", "chatgpt", "gpt-5.6-sol"
        ),
        exception_status=500,
